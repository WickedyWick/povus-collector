...
var survey_pin_color_toggle = [ '#FF0000', '#00FFFF' ]; // 0 red // 1
var survey_label_color_toggle = [ '#FF0000', '#00A0A0' ];
let numOfPins = 0
let pinObjects = new Map()
let pinLabelObjects = new Map()
let timer = false
// Initialize page when loaded.
$( function() {
  
  setInterval(() => {
    for (let [key, value] of pinLabelObjects) {
      let time = $(`#survey_dot_label_${key}`).html()
      if (time > 0) {
        time-- 
        $(`#survey_dot_label_${key}`).html(time)
      }
      if ( time == 0 ) {
        $(`#survey_dot_label_${key}`).css('color', survey_label_color_toggle[1])
        $(`#survey_dot_${key}`).css('color', survey_label_color_toggle[1])
        /*
        value--// put this on click, so users can change or force remove it
        if ( value == 0 ) {
          //remove it
          $(`#survey_dot_${key}`).remove()
          $(`#survey_dot_label_${key}`).remove()
          survey_dot_positions.splice(key, 1)
          survey_dot_label_positions.splice(key, 1)
        } */
      }
      
  }},1000)

  $('#zone_map').click(function(e) {

    let survey_pins_html = $('#regular_survey_pins').html()
    survey_pins_html = survey_pins_html + '<span class="survey_dot" id ="survey_dot_' + numOfPins
      + '" data-index="' + numOfPins + '" style="top: 0px; left: 0px; position: absolute;"></span>' 
      + '<label for="dot_' + numOfPins + '" class="survey_dot_label" id="survey_dot_label_' + numOfPins 
      + '" data-index="' + numOfPins + '" style="top: 0px; left: 0px; position: absolute;" script="alert("test")">'
      + 180 + '</label>'; 
      $('#regular_survey_pins').html(survey_pins_html);
      
      player_x = 15 + e.offsetX
      player_y= e.offsetY

      let dotLabel = $('#survey_dot_label_' + numOfPins)
      let dot = $('#survey_dot_' + numOfPins)
      dot.css({ left: player_x + 'px', top: player_y+ 'px' });
      dotLabel.css({ left: player_x + 'px', top: player_y + 'px' });
      pinObjects.set(numOfPins, 2) // not needed
      pinLabelObjects.set(numOfPins, 2)
      console.log(pinLabelObjects)
      survey_dot_positions.push([player_x, player_y]);
      survey_dot_label_positions.push([player_x, player_y]);
      numOfPins++
      register()
      /*
      dotLabel.click((e) => {
        let id = Number(dotLabel.attr('id').split('_')[3])
        console.log(id)
        pinLabelObjects.set(id, pinLabelObjects.get(id) - 1)
        if( pinLabelObjects.get(id) == 0) {
          $(`#survey_dot_${id}`).remove()
          dotLabel.remove()
          survey_dot_positions.splice(id, 1)
          survey_dot_label_positions.splice(id, 1)
        } else {  
          dotLabel.html("180")
        }
        ...
        */
      //console.log(survey_dot_positions)
  })
...
function harvested(pin) {
  let id = Number($(pin).attr('id').split('_')[3])
 // console.log(pinLabelObjects)
  pinLabelObjects.set(id, Number(pinLabelObjects.get(id)) - 1)
  if( pinLabelObjects.get(id) == 0) {
    $(`#survey_dot_${id}`).remove()
    $(pin).remove()
    survey_dot_positions.splice(id, 1)
    ...
    $(pin).css('color', survey_label_color_toggle[0])
    $(`#survey_dot_${id}`).css('color', survey_label_color_toggle[0])
    $(pin).html("180")
...
function update_regular_survey_pins() {
  ...
  var player_x_meters = 0.5 * map_size_x + player_icon_size[0]/2;
  var player_y_meters = 0.5 * map_size_y - player_icon_size[1]/2;

  for(var ii = 0; ii < numOfPins; ii++) {
    survey_dot_positions[ii][0] = survey_dot_positions[ii][0] / width * map_image_width +15
    survey_dot_positions[ii][1] = survey_dot_positions[ii][1] / height * map_image_height
    /*
    ...
    */
    ...
    if(survey_dot_positions[ii][0] > map_size_x) { survey_dot_positions[ii][0] = map_image_width; };
    if(survey_dot_positions[ii][1] > map_size_y) { survey_dot_positions[ii][1] = map_image_height; };
    /*
